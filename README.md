...
| 👀 | [SandBase Harness](https://github.com/sandbaseai/sandbase-harness) | mcp, sandbox, sessions, self-hosted | Local-first, self-hosted AI agent runtime and MCP bridge with sandboxed sessions, memory, credentials, audit/replay, and a local Console |
...
| 👀 | [Agentlas OS](https://github.com/agentlas-ai/Agentlas-OS) | agent-os, orchestration, local-first, multi-model | Agent OS: keep specialist agents in a hub, spin up a temporary orchestrator per task. Local-first, works with any model. |
...
| 👀 | [pi-custom-packages](https://github.com/Blue-B/pi-custom-packages) | pi, extension-pack, wsl, screenshot, session-guards | Ten extensions for the pi coding agent: session guards, image context control, and Windows/WSL screen capture, cursor, and recording. |
| 👀 | [Agent Island](https://github.com/tristan666666/agent-island) | macos, windows, status, local-first | Open-source status companion for Claude Code, Codex, Antigravity, Grok and Cursor with live session state, your-turn alerts, and locally computed usage and cost |
| 👀 | [termagitchi](https://github.com/TevvvB/termagitchi) | status-line, worktrees, gacha, git-hygiene | A creature for every git worktree, living in your coding agent's status line. Deterministic species, rarity bands, and one view across every live session. |
| 👀 | [agent-watch](https://github.com/soul-sol/agent-watch) | monitoring, stall-detection, preflight, posix-sh | Monitor Claude Code and Codex CLI agents for running, completed, failed, or stalled jobs. |
| 👀 | [Superagent](https://github.com/pungme/superagent-desktop) | macos, browser, ios-simulator, phone-relay | Desktop app giving Claude Code and Codex a real browser to drive, an iOS Simulator to install and screenshot apps in, and a phone companion app for remote monitoring |
...
| 👀 | [Claudette](https://github.com/Olorin-ai-git/claudette) | mobile, control-plane, pty, ios, android | Native iOS/Android/Apple TV mobile control plane for local CLI coding-agent sessions: real PTY plus context/cost gauge, agent tree, voice, and Take the Wheel handoff; companion CLI `npx claudette setup` |
...
| 👀 | [ctxtrim](https://github.com/AgentPostmortem/Ctxtrim) | context, token-saving, cli | Trim what bloats your AI coding context — find the files ballooning your Claude Code / Cursor / Codex token cost and write ignore files to cut it. Zero-dep. npx ctxtrim |
